...
ARGS=("../src" "small_directory" "empty_directory" "small_directory2" "src-bigfiles" "src-smallfiles")
make all > /dev/null
for dir in "${ARGS[@]}"; do

    echo "Testing  on $dir"

    count1=$(./fauxgrep hi "$dir" | wc -w)
    count2=$(./fauxgrep-mt hi "$dir" | wc -w)

    if [[ "$count1" -eq "$count2" ]]; then
        echo "Test passed: same number of matching words ($count1)"
    else
        echo "Test failed: line counts differ (orig=$count1, mt=$count2)"
    fi

   # --- Measure average execution times (100 runs) ---
runs=10
total1=0
total2=0
